...
| Everything offline | Core exercise logic is offline; Claude features need the internet | Rep counting and safety rules stay local. In the hackathon build, voice intents, generated feedback, and Echo's fallback call Claude directly with a bundled demo key. Echo prefers on-device Gemma. |
